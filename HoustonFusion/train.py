...
class GAN():
    def __init__(self):
        ...
        self.classifier = load_model('/home/SharedData/Avinandan/TeacherStudentExperiments/HoustonFusion/twostream.h5')
        self.streamone = Model(inputs=self.classifier.get_layer('Houston_Fusion_Input_1').input,
                               outputs=self.classifier.get_layer('Houston_Fusion_Output_1').output)
        self.streamtwo = Model(inputs=self.classifier.get_layer('Houston_Fusion_Input_2').input,
                               outputs=self.classifier.get_layer('Houston_Fusion_Output_2').output)
    ...
    def train(self, epochs, batch_size=128):
        ...
        X_train, gt = createPatches(X_train, gt, windowSize=3, removeZeroLabels=False)

        #X_train=np.reshape(X_train,(21025,200))
        X_train_one = X_train[:,:,:,0:100]
        X_train_two = X_train[:,:,:,100:144]
                ...
                if epoch >= epochs/2 and epoch%20==0:
                    ...
                    #self.discriminator.save('/home/SharedData/Avinandan/TeacherStudentExperiments/IndianPines/2N/Concatenated/TemperatureExperiments/T3/models/discriminator2N-{0}.h5'.format(epoch))
                     ...
                     if results1[1]>topacc:
                          self.generator.save('/home/SharedData/Avinandan/TeacherStudentExperiments/HoustonFusion/generator.h5')
                          topacc = results1[1]
