...
class GAN():
    def __init__(self):
        ...
        self.classifier = load_model('/home/SharedData/Avinandan/TeacherStudentExperiments/PaviaUniv/2N/Concatenated/twostreamconcat.h5')
        self.streamone = Model(inputs=self.classifier.get_layer('Pavia_Univ_Input_1').input,
                               outputs=self.classifier.get_layer('Pavia_Univ_Output_1').output)
        self.streamtwo = Model(inputs=self.classifier.get_layer('Pavia_Univ_Input_2').input,
                               outputs=self.classifier.get_layer('Pavia_Univ_Output_2').output)
    ...
    def build_discriminator(self):
        ...
        model.add(Dense(18, activation='softmax', name='Classifier_Output'))
        #model.summary()
    ...
    def train(self, epochs, batch_size=128):
        ...
        X_train_one = X_train[:,:,:,0:50]
        X_train_two = X_train[:,:,:,50:103]
        ...
        (X_train_one, X_test_one,Y,Y_test,) =    train_test_split(X_train_one,gt, random_state = 666 , test_size=0.75)

        (X_train_two, X_test_two,Y,Y_test,) =    train_test_split(X_train_two,gt, random_state = 666 , test_size=0.75)
        ...
        for epoch in range(epochs):
            ...
            for g in range(5):
                ...
                if epoch >= epochs/2 and epoch%20==0:
                    #self.generator.save('/home/SharedData/Avinandan/TeacherStudentExperiments/IndianPines/2N/Concatenated/TemperatureExperiments/T3/models/generator2N-{0}.h5'.format(epoch))
                    #self.discriminator.save('/home/SharedData/Avinandan/TeacherStudentExperiments/IndianPines/2N/Concatenated/TemperatureExperiments/T3/models/discriminator2N-{0}.h5'.format(epoch))
                     ...
                     X_gen = np.concatenate((generated_images,X_test_two),axis=1)

                     results1 = self.finalclassifier.evaluate(X_gen,Y_test, batch_size=32, verbose=1)
                     print(results1[1])

                     if results1[1]>topacc:
                          self.generator.save('/home/SharedData/Avinandan/TeacherStudentExperiments/PaviaUniv/2N/Concatenated/generator.h5')
                          topacc = results1[1]
